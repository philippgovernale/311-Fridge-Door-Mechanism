...
#define 10_DUTY_CYCLE_ON_T 5 /*given freq of 20 Hz (0.1 * 1/20)*/
#define 10_DUTY_CYCLE_OFF_T 45
...
/*OBSOLETE*/
void finish_sleep(){
	CLKPR &= ~(1 << CLKPS2); //return to 16 Mhz clock
}
...
ISR(TIMER_0_COMPA_VECT){
	timer_count_open_PWM++;
	
	if (PWM_state_high && timer_count_open_PWM == 10_DUTY_CYCLE_ON_T){
		switch(0, DOOR_OPEN);
		timer_count_open_PWM = 0;
		PWM_state_high = 0;
	}
	
	else if (!PWM_state_high && timer_count_open_PWM == 10_DUTY_CYCLE_OFF_T){
		switch(1, DOOR_OPEN);
		timer_count_open_PWM = 0;
		PWM_state_high = 1;
	}
